...
mon_pr_nc = netCDF4.Dataset('miroc3_2_medres/pcmdi.ipcc4.miroc3_2_medres.20c3m.run1.monthly.pr_A1.nc')
...
mon_i = numpy.digitize([nt.sts[stid]['lon'] + 360.,], mon_lons)
print i, j, mon_i, mon_j
...
for yr in range(1981, 2000):
  ...
  data.append( numpy.sum(pr_nc.variables['Prcp'][0,idx1:idx2,j,i]) / 25.4 * 30.4)
  data2.append( numpy.sum(pr_nc2.variables['Prcp'][0,idx1:idx2,j,i]) / 25.4 * 30.4 )
  ...
  data3.append( numpy.sum(mon_pr_nc.variables['pr'][idx1:idx2,mon_j,mon_i]) / 25.4 * 86400.0 * 31.0)

(fig, ax) = plt.subplots(1,1)

ax.bar(numpy.arange(1981,2000)-0.45, data3, fc='g', ec='g', width=0.3, label='MIROC3_2 $2^\circ$ PCMDI')
ax.bar(numpy.arange(1981,2000)-0.15, data, fc='r', ec='r', width=0.3, label=r'Gridded $2^\circ$ Obs')
ax.bar(numpy.arange(1981,2000)+0.15, data2, fc='b', ec='b', width=0.3, label='MIROC3_2 Bias Correct')
