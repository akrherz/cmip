...
obs = numpy.average(obs_nc.variables['Tavg'][offset1:offset2,:,:],0)
fut = numpy.average(fut_nc.variables['Tavg'][offset3:offset4,:,:],0)

ratio = fut - obs
#ratio.mask = numpy.where(ratio == 1, True, False)
print  numpy.min(ratio), numpy.max(ratio)
m = MapPlot(sector='conus', caption='', nologo=True,
            title=r'MIROC3.2 exp1 BCSD Annual Temp Difference A1B minus C20',
...
m.postprocess(filename='annual_tdiff_bcsd_miroc_only.png')
