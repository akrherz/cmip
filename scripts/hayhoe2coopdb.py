...
pr_nc = netCDF4.Dataset(
    ("%s/%s.%s.pr.NAm.grid.1960.2099.nc") % (BASE, model, scenario)
)
tasmax_nc = netCDF4.Dataset(
    ("%s/%s.%s.tmax.NAm.grid.1960.2099.nc") % (BASE, model, scenario)
)
tasmin_nc = netCDF4.Dataset(
    ("%s/%s.%s.tmin.NAm.grid.1960.2099.nc") % (BASE, model, scenario)
)
...
if (np.shape(tmdata)[0] / 365.0) != 140:
    print(
        ("FAILURE, %s years found, not exactly 140")
...
def do(lon, lat, station):
    ...
    for k, _ in enumerate(tmdata):
        ...
        if low is not None and high is not None and low > high:
            # Swap, sigh
            print(
                ("%s %s high: %.1f low: %.1f was swapped")
...
do(-111.97, 33.0589, "BDMARI")
do(-103.214, 34.601, "BDCLOV")
do(-106.764, 34.766, "BDLOSL")
do(-103.695, 35.194, "BDTUCU")
do(-119.803, 39.572, "BDRENO")
do(-118.7857, 39.4578, "BDFALL")
