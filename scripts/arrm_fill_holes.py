...
for sid in ["BDMARI", "BDCLOV", "BDLOSL", "BDTUCU", "BDRENO", "BDFALL"]:
    ...
    for varname in ["high", "low", "precip"]:
        cursor2.execute(
            ...
            lag("""
            + varname
            + """) OVER (ORDER by day ASC) as la,
            lead("""
            + varname
            + """) OVER (ORDER by day ASC) as le
        ...
        for row2 in cursor2:
            if varname == "precip":
                newval = "%.2f" % ((row2["la"] + row2["le"]) / 2.0)
            else:
                newval = int((row2["la"] + row2["le"]) / 2.0)
            cursor3.execute(
                """UPDATE hayhoe_daily SET """
                + varname
                + """ = %s
                WHERE model = %s and scenario = %s and station = %s
                and day = %s""",
                (newval, MODEL, SCENARIO, sid, row2["day"]),
            )
        if cursor2.rowcount > 0:
            print "SID: %s varname: %-7s count: %-4s" % (
                sid,
                varname,
                cursor2.rowcount,
            )
    ...
    cursor.execute(
        """SELECT day, high, low, precip from hayhoe_daily where
    model = %s and scenario = %s and station = %s and
    (high is null or low is null or precip is null)""",
        (MODEL, SCENARIO, sid),
    )
    cnt = cursor.rowcount
    if cnt > 0:
        ...
        if cnt < 20:
            for row in cursor:
                ...
                if row[1] is None:
                    cursor2.execute(
                        """SELECT avg(high) from hayhoe_daily where
                    model = %s and scenario = %s and day = %s and
                    high is not null""",
                        (MODEL, SCENARIO, row[0]),
                    )
                ...
                if newhigh < newlow:
                    ...
                    newlow = newlow2
                cursor2.execute(
                    """UPDATE hayhoe_daily SET high = %s, low = %s,
                precip = %s WHERE model = %s and scenario = %s and day = %s
                and station = %s""",
                    (newhigh, newlow, newprecip, MODEL, SCENARIO, row[0], sid),
                )
