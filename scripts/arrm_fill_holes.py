            ...
            for row in cursor:
                ...
                if row[1] is None:
                    ...
                    if newhigh is None:
                        ...
                        cursor2.execute("""SELECT avg(high) from hayhoe_daily where
                        model = %s and scenario = %s and day = %s and 
                        high is not null""", (MODEL, SCENARIO, 
                ...
                if row[2] is None:
                    ...
                    if newlow is None:
                        ...
                        cursor2.execute("""SELECT avg(low) from hayhoe_daily where
                        model = %s and scenario = %s and day = %s and 
                        low is not null""", (MODEL, SCENARIO, 
                                              row[0] - datetime.timedelta(days=1)))
                        newlow = cursor2.fetchone()[0]
                        if newlow is None:
                            ...
                            continue
                if row[3] is None:
                    cursor2.execute("""SELECT avg(precip) from hayhoe_daily where
                    model = %s and scenario = %s and day = %s and 
                    ...
                    if newprecip is None:
                        ...
                        cursor2.execute("""SELECT avg(precip) from hayhoe_daily where
                        model = %s and scenario = %s and day = %s and 
                        precip is not null""", (MODEL, SCENARIO, 
                                              row[0] - datetime.timedelta(days=1)))
                        newprecip = cursor2.fetchone()[0]
                        if newprecip is None:
                ...
                print "%s %10s %10s %10s --> %5.1f %5.1f %5.2f" % (row[0], row[1],
                    row[2], row[3], newhigh, newlow, newprecip) 
                if newhigh < newlow:
                    newlow2 = newhigh
                    newhigh = newlow
                    newlow = newlow2
