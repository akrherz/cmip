...
for i, mon in enumerate(
    ...
    print mon
    obs = numpy.average(
        obs_nc.variables["Prcp"][offset1 + i : offset2 : 12, :, :], 0
    )
    fut = numpy.average(
        fut_nc.variables["Prcp"][offset3 + i : offset4 : 12, :, :], 0
    )
    ...
    m = MapPlot(
        ...
        caption="",
        nologo=True,
        title=r"MIROC3.2 exp1 BCSD [%s] Monthly Precipitation Ratio A1B / C20"
        % (mon,),
    ...
    m.postprocess(
        filename="monthly_ratio_bcsd_%s_miroc_only.png" % (mon.lower(),)
    )
